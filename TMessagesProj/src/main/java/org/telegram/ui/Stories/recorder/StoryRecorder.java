...
//    private StoryPrivacySelector.StoryPrivacyHint privacySelectorHint;
    ...
    private void initViews() {
        ...
        containerView.addView(actionBarContainer = new FrameLayout(context) {
            ...
            @Override
            protected void dispatchDraw(Canvas canvas) {
                blurDrawer.drawRect(canvas, 0, 0, getAlpha(), true);
                super.dispatchDraw(canvas);
            }
        }); // 150dp
            ...
            public void onSurfaceTextureUpdated(android.graphics.SurfaceTexture surface) {
                ...
                if (cameraHUDView != null && cameraView != null) {
                    ...
                    if (session != null) {
                        ...
                        } else if (session.camera1Session != null && session.camera1Session.cameraInfo.getCamera() != null) {
                            try {
                                cameraHUDView.updateMetadata(session.camera1Session.cameraInfo.getCamera().getParameters());
                            } catch (Exception ignore) {}
                        }
                    }
                }
...
//            });
    ...
    private void updateActionBarButtons(boolean animated) {
        ...
        AndroidUtilities.updateViewShow(cameraHUDView, !animatedRecording && currentPage == PAGE_CAMERA && !collageListView.isVisible() && !inCheck(), true, animated);
