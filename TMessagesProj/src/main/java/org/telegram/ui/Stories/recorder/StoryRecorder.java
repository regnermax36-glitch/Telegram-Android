...
//                    captionEdit.editText.hideEmojiView();
        ...
        protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
            ...
            int l = (W + insetLeft - insetRight - previewW) / 2,
                r = (W + insetLeft - insetRight + previewW) / 2;
            int t = statusbar + (H - statusbar - insetBottom - previewH - underControls) / 2;
            int b = t + previewH + underControls;
    ...
    private void initViews() {
        ...
        containerView.addView(previewContainer = new FrameLayout(context) {
            ...
            protected void dispatchDraw(@NonNull Canvas c) {
                ...
                c.drawRoundRect(AndroidUtilities.rectTmp, dp(32), dp(32), borderPaint);
            }
        });
...
//                privacySelector.setStoryPeriod(period);
        ...
        startLiveButton.setScaleX(0.85f);
        startLiveButton.setScaleY(0.85f);
...
//                    storyEntry.setupMatrix();
    ...
    private void updateHUD() {
        ...
        if (session.isCamera2()) {
            ...
            if (result != null) {
                ...
                Integer aeExp = result.get(CaptureResult.CONTROL_AE_EXPOSURE_COMPENSATION);
                Float aeStep = s.getCameraCharacteristics().get(CameraCharacteristics.CONTROL_AE_COMPENSATION_STEP).floatValue();

                String sec = "N/A";
                if (exposureTime != null) {
                    double sVal = exposureTime / 1_000_000_000.0;
                    if (sVal < 1.0) {
                        sec = "1/" + Math.round(1.0 / sVal);
                    } else {
                        sec = String.format(Locale.US, "%.1fs", sVal);
                    }
                }
                cameraHUDView.updateMetadata(String.valueOf(iso), String.format(Locale.US, "%+.1f", aeExp * aeStep), sec);
            }
        } else {
            ...
            if (camera != null) {
                Utilities.globalQueue.postRunnable(() -> {
                    try {
                        ...
                        String finalIso = iso;
                        AndroidUtilities.runOnUIThread(() -> {
                            cameraHUDView.updateMetadata(finalIso, String.format(Locale.US, "%+.1f", ev), "N/A");
                        });
                    } catch (Exception ignore) {}
                });
            }
    ...
    private void onNavigateStart(int fromPage, int toPage) {
        if (toPage == PAGE_CAMERA) {
            ...
            cameraHUDView.setVisibility(View.VISIBLE);
...
//                recordControl.setDual(isDual());
        ...
        cameraView.setOnSurfaceTextureUpdatedListener(this::updateHUD);
